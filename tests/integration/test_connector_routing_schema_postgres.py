...
def connector_database(
    ...
    try:
        ...
        assert upgraded.applied_versions == (9, 10, 11, 12, 13, 14)
...
def test_pristine_schema_applies_versions_one_through_fourteen() -> None:
    database = f"schemabridge_connector_pristine_{uuid4().hex[:12]}"
    urls = _create_database(database)
    try:
        migrated = PostgresControlPlaneMigrator(urls.migrator, MIGRATIONS).migrate()
        assert migrated.applied_versions == tuple(range(1, 15))
        assert migrated.inspection.current_version == 14
        with psycopg.connect(urls.migrator) as connection:
            tables = {
                str(row[0])
