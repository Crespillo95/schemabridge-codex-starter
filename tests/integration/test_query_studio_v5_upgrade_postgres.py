...
def test_v5_to_v12_preserves_state_and_keeps_runtime_non_migrating(
    ...
    try:
        ...
        assert upgraded.applied_versions == (6, 7, 8, 9, 10, 11, 12, 13, 14)
        ...
        assert current_runtime.current_version == 14
...
def test_populated_v6_success_failure_and_expiry_upgrade_to_v12_with_exact_acl(
    ...
    try:
        ...
        assert upgraded_release.applied_versions == (8, 9, 10, 11, 12, 13, 14)
        assert upgraded_release.inspection.current_version == 14
...
def test_v8_aborts_on_invalid_audit_derivation_without_rewriting(
    ...
    try:
        ...
        assert tuple(item.version for item in pending.pending) == (8, 9, 10, 11, 12, 13, 14)
    finally:
        _drop_database(database)
